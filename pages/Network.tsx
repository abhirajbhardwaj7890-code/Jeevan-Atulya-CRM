                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl w-full max-w-2xl overflow-hidden animate-fade-in shadow-2xl flex flex-col max-h-[90vh]">
                        <div className="bg-slate-900 px-6 py-4 flex justify-between items-center text-white shrink-0">
                            <h3 className="font-bold flex items-center gap-2">
                                <Users size={18} />
                                {agents.find(a => a.id === viewAgentId)?.name}'s Members
                            </h3>
                            <button onClick={() => setViewAgentId(null)} className="text-slate-400 hover:text-white"><X size={20} /></button>
                        </div>

                        <div className="p-0 overflow-y-auto flex-1">
                            {members.filter(m => m.agentId === viewAgentId).length > 0 ? (
                                <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody>
                                        {members.filter(m => m.agentId === viewAgentId).map(member => (
                                ...
                                <div className="p-12 text-center text-slate-400">
                                    ...
                                    <p>No members assigned to this agent yet.</p>
                                </div>
                            )}
                        </div>

                        <div className="p-4 border-t border-slate-100 bg-slate-50 shrink-0 flex justify-end">
                            <button onClick={() => setViewAgentId(null)} className="px-6 py-2 bg-slate-900 text-white rounded-lg font-bold hover:bg-slate-800">Close</button>
                        </div>
                    </div>
                </div>
            )}
