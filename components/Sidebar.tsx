...
export const Sidebar: React.FC<SidebarProps> = ({ activePage, userRole, onNavigate, onSwitchRole, onLogout, onRefresh }) => {
  const navItems = [
      ...
      <div className="p-4 border-t border-slate-800 space-y-4">
        ...
        <div>
            {onRefresh && (
                <button 
                  onClick={onRefresh}
                  className="w-full flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-slate-800 transition-colors text-slate-400 mb-1"
                >
                    <RefreshCw size={20} />
